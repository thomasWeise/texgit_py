...
VERSION := $(shell (less '$(CWD)/latexgit/version.py' | sed -n 's/__version__.*=\s*"\(.*\)"/\1/p'))
...
clean: status
	echo "$(NOW): Cleaning up by first uninstalling latexgit (if installed) and then deleting all auto-generated stuff." && \
	pip uninstall -y latexgit || true && \
	echo "$(NOW): latexgit is no longer installed; now deleting auto-generated stuff." && \
	rm -rf *.whl && \
...
test: init
	...
	timeout --kill-after=15s 90m coverage run -a --include="latexgit/*" -m pytest --strict-config --doctest-modules --ignore=tests && \
	echo "$(NOW): Running pytest tests." && \
	timeout --kill-after=15s 90m coverage run --include="latexgit/*" -m pytest --strict-config tests && \
...
static_analysis: init
	...
	pylint latexgit --disable=C0103,C0302,C0325,R0801,R0901,R0902,R0903,R0911,R0912,R0913,R0914,R0915,R1702,R1728,W0212,W0238,W0703 &&\
	echo "$(NOW): Done with pylint, now trying mypy." &&\
	mypy latexgit --no-strict-optional --check-untyped-defs &&\
	echo "$(NOW): Done with mypy, now doing pyflakes." &&\
	python3 -m pyflakes . &&\
	echo "$(NOW): Done with pyflakes, now applying bandit to find security issues." &&\
	bandit -r latexgit -s B311 &&\
	...
	tryceratops -i TRY003 -i TRY101 latexgit &&\
...
create_documentation: static_analysis test
	...
	sphinx-apidoc -M --ext-autodoc -o docs/source ./latexgit && \
	...
	coverage html -d docs/build/tc --include="latexgit/*" &&\
	...
	export BASE_URL='https\:\/\/thomasweise\.github\.io\/latexgit\/' &&\
	echo "$${PART_A}Contributing to latexgit$${PART_B}$(shell (python3 -m markdown -o html ./CONTRIBUTING.md))$$PART_C" > ./docs/build/CONTRIBUTING.html &&\
	sed -i "s/\"$$BASE_URL/\".\//g" ./docs/build/CONTRIBUTING.html &&\
	sed -i "s/=$$BASE_URL/=.\//g" ./docs/build/CONTRIBUTING.html &&\
	echo "$${PART_A}Security Policy of latexgit$${PART_B}$(shell (python3 -m markdown -o html ./SECURITY.md))$$PART_C" > ./docs/build/SECURITY.html &&\
...
create_distribution: static_analysis test create_documentation
	...
	python3 -m pip --no-input --timeout 360 --retries 100 --require-virtualenv install "$(CWD)/dist/latexgit-$(VERSION).tar.gz" && ## nosem \
	echo "$(NOW): Installing tar.gz has worked. We now create the list of packages in this environment via pip freeze." &&\
	pip freeze > "$(CWD)/dist/latexgit-$(VERSION)-requirements_frozen.txt" &&\
	echo "$(NOW): Now fixing latexgit line in requirements file." &&\
	sed -i "s/^latexgit.*/latexgit==$(VERSION)/" "$(CWD)/dist/latexgit-$(VERSION)-requirements_frozen.txt" &&\
	...
	python3 -m pip --no-input --timeout 360 --retries 100 --require-virtualenv install "$(CWD)/dist/latexgit-$(VERSION)-py3-none-any.whl" && ## nosem \
	echo "$(NOW): Now we deactivate the environment." &&\
	deactivate &&\
	...
	cd docs/build &&\
	tar --dereference --exclude=".nojekyll" -c * | xz -v -9e -c > "$(CWD)/dist/latexgit-$(VERSION)-documentation.tar.xz" &&\
	cd $(CWD) &&\
...
install: create_distribution
	echo "$(NOW): Now installing latexgit." && \
	pip --no-input --timeout 360 --retries 100 -v install . && \
	echo "$(NOW): Successfully installed latexgit."
